...
class User{
    ...
    static async register(credentials){
        ...
        const requiredFields = ["email","username","first_name","last_name", "password", "age", "zip_code"]
        requiredFields.forEach(field =>{
            if(!credentials.hasOwnProperty(field)){
                throw new BadRequestError(`Missing ${field} in request body`)
            }
        ...
        const result = await db.query(
           ` INSERT INTO users (
            ...
            RETURNING id, email, username;`, [lowercasedEmail, credentials.username, credentials.zip_code,
